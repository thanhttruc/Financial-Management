...
import { updateAccount, AccountType } from '../api/accounts';
import type { AccountDetails } from '../api/accounts';
 ...
 * Form được điền sẵn với dữ liệu hiện tại của tài khoản
 */
export const EditAccountForm: React.FC<EditAccountFormProps> = ({ accountDetails, onSuccess, onCancel }) => {
  const { toast, showToast, hideToast } = useToast();
  const navigate = useNavigate();
  
  // State cho các trường input - được điền sẵn từ accountDetails
  const [formData, setFormData] = useState({
    bankName: accountDetails.bank_name || '',
    accountType: accountDetails.account_type,
    branchName: accountDetails.branch_name || '',
    accountNumberFull: accountDetails.account_number_full || '',
  ...
  // Cập nhật formData khi accountDetails thay đổi
  useEffect(() => {
    setFormData({
      bankName: accountDetails.bank_name || '',
      accountType: accountDetails.account_type,
  ...
  /**
   * Hàm xử lý lưu thay đổi - gửi request PUT để cập nhật tài khoản
   */
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    if (isNaN(balanceNum) || balanceNum < 0) {
      newErrors.balance = 'Số dư phải là số và lớn hơn hoặc bằng 0';
    }
    ...
    try {
      ...
      // Xử lý lỗi 403/404 - Bảo mật hoặc không tồn tại
      else if (status === 403 || status === 404) {
        showToast('Truy cập bị từ chối hoặc Tài khoản không tồn tại.', 'error');
        // Điều hướng về trang danh sách tài khoản sau 2 giây
        setTimeout(() => {
          navigate('/accounts');
        }, 2000);
      }
      ...
      // Xử lý các lỗi khác
        ...
        const errorMessage = errorData?.message || 'Đã xảy ra lỗi khi cập nhật tài khoản';
        showToast(errorMessage, 'error');
      }
    } finally {
      setIsLoading(false);
  ...
  return (
    <>
      <Card className="max-w-2xl mx-auto">
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Chỉnh sửa chi tiết tài khoản</h2>
